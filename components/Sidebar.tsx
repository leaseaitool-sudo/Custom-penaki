...
import { View, User, Role, Lease, LeaseStatus } from '../types';
    ...
    if (isAdmin) {
      return (
        <>
          ...
          {isSuperAdmin && <NavItem view="admin-bookings" label="Demo Bookings" icon={<CalendarIcon className="w-6 h-6" />} activeView={activeView} onClick={handleNavigation} />}
    ...
    if (isAppMode) {
        return (
            <>
                ...
                <NavItem view="reminders" label="Reminders" icon={<BellIcon className="w-6 h-6" />} activeView={activeView} onClick={handleNavigation} badge={reminderCount} />
