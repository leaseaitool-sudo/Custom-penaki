...
import { User, SavedTemplate, SelectionSection } from '../types';
...
export const ProfilePage: React.FC<ProfilePageProps> = ({ user, onUpdateUser, onDeleteTemplate, onUpdateTemplate }) => {
  ...
  const Message: React.FC<{ message: { type: string; text: string } }> = ({ message }) => {
    ...
    const classes = isError ? 'bg-red-100 border-red-300 text-red-800' : 'bg-green-100 border-green-300 text-green-800';
    return (
        ...
        <form onSubmit={handleInfoSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                <input type="text" id="username" value={username} onChange={(e) => setUsername(e.target.value)} className={commonInputClasses} />
              </div>
              <div>
          ...
          <div className="flex justify-end">
            <button type="submit" className={commonButtonClasses}>Save Changes</button>
          </div>
        </form>
      </div>
      ...
      <div className="bg-surface p-8 rounded-2xl border border-border shadow-lg">
        ...
        <form onSubmit={handlePasswordSubmit} className="space-y-6">
          ...
          <div>
            ...
            <input type="password" id="current-password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} className={commonInputClasses} placeholder="••••••••" />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <label htmlFor="new-password" className="block text-sm font-medium text-text-light">New Password</label>
              <input type="password" id="new-password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className={commonInputClasses} placeholder="••••••••" />
            </div>
            <div>
              <label htmlFor="confirm-password" className="block text-sm font-medium text-text-light">Confirm New Password</label>
              <input type="password" id="confirm-password" value={confirmPassword} onChange={(e) => setConfirmPassword(e.target.value)} className={commonInputClasses} placeholder="••••••••" />
            </div>
          </div>
           <div className="flex justify-end">
            <button type="submit" className={commonButtonClasses}>Update Password</button>
          </div>
        </form>
      </div>
