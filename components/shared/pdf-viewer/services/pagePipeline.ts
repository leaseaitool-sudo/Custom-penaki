...
export const processPagePipeline = async (
    ...
    try {
        ...
        const renderTask = await renderPageCanvas(page, scale, canvas);
        ...
        if (!textItems) {
            ...
            if (cache.textPromises.has(pageKey)) {
                ...
                const promise = extractPageText(docIndex, pageIndex, doc, canvas, onOcrProgress);
                cache.textPromises.set(pageKey, promise);
                textItems = await promise;
                cache.textPromises.delete(pageKey);
            }
        ...
        // 4. Highlights
        const snippetHash = snippets.length || 0;
        const highlightCacheKey = `${pageKey}-${snippetHash}`;
