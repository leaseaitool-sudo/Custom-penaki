...
import { User, Role, Lease, ReviewStatus } from '../../types';
...
export const AdminReviewers: React.FC<AdminReviewersProps> = ({ 
  ...
  const handleAddSubmit = (e: React.FormEvent) => {
    ...
    if (reviewers.some(r => r.email.toLowerCase() === newReviewerEmail.toLowerCase())) {
        setError('Reference collision: Email already exists.'); return;
    }

    onAddReviewer({
        username: newReviewerName, email: newReviewerEmail, password: newReviewerPassword,
        role: Role.REVIEWER, isOnline: false, dailyCapacity: 10, dailyGoal: 5
    });
    setNewReviewerName(''); setNewReviewerEmail(''); setNewReviewerPassword('');
  ...
  return (
    <ScrollAnimatedSection className="space-y-10 max-w-[95rem] mx-auto px-4 pb-24">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
          ...
          <div className="bg-white p-8 rounded-3xl border border-border shadow-md hover:shadow-lg transition-all group">
              <h4 className="text-[10px] font-extrabold text-slate-400 uppercase tracking-widest mb-2 group-hover:text-green-500 transition-colors">Nodes Online</h4>
              <p className="text-4xl font-black text-green-600 tracking-tighter">{reviewers.filter(r => r.isOnline).length}</p>
          </div>
          <div className="bg-white p-8 rounded-3xl border border-border shadow-md hover:shadow-lg transition-all group">
                            ...
                            <div className="p-6 flex flex-col md:flex-row items-center gap-8">
                                <div className="flex items-center gap-5 min-w-[280px]">
                                    <div className="relative">
                                        ...
                                        <div className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-4 border-white ${reviewer.isOnline ? 'bg-green-500 shadow-[0_0_10px_rgba(34,197,94,0.5)]' : 'bg-slate-300'}`}></div>
                                    </div>
                                ...
                                <div className="flex-1 flex flex-wrap gap-12 justify-center md:justify-start">
                                    <div className="text-center md:text-left">
                                        ...
                                        <span className={`inline-flex items-center gap-2 text-xs font-black uppercase ${reviewer.isOnline ? 'text-green-600' : 'text-slate-400'}`}>
                                            <span className={`w-2 h-2 rounded-full ${reviewer.isOnline ? 'bg-green-500 animate-pulse' : 'bg-slate-300'}`}></span>
                                            {reviewer.isOnline ? 'Active Session' : 'Offline'}
                                        </span>
                                    </div>
                                    <div className="text-center md:text-left">
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <button 
                                        onClick={(e) => { e.stopPropagation(); if(window.confirm(`Decommission reviewer account: ${reviewer.username}? All active jobs will be unassigned.`)) onDeleteReviewer(reviewer.email); }}
                                        className="p-3 bg-red-50 text-red-400 hover:text-red-600 hover:bg-red-100 rounded-2xl transition-all border border-red-50 hover:shadow-md" title="Decommission"
                                    >
