...
import { Lease, ReviewStatus, User, LeaseStatus, Role, WorkflowStage } from '@/shared/types';
...
import { fetchReviewerMetrics } from '@/features/leases/api/leaseService';
...
export const ReviewerDashboard: React.FC<ReviewerDashboardProps> = ({ leases, currentUser, onOpenWorkbench, onNavigateToActivity }) => {
    ...
    useEffect(() => {
        ...
        const loadMetrics = async () => {
            try {
                const data = await fetchReviewerMetrics(currentUser.email);
                if (mounted) setMetrics(data);
            } catch (err) {
                console.error("Failed to load reviewer metrics", err);
            }
        };
        loadMetrics();
    ...
    // 2. Local Queue (We keep this for rendering the actual cards)
    ...
    return (
        <ScrollAnimatedSection className="space-y-8 max-w-[95rem] mx-auto px-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
                <WidgetCard title="Task R1 Queue" value={metrics.r1_pending} icon={<DocumentTextIcon />} subtext="First pass required" colorClass="text-green-600" onClick={() => onNavigateToActivity('InProgress')} />
                <WidgetCard title="Task R2 Queue" value={metrics.r2_pending} icon={<DocumentTextIcon />} subtext="Final verification required" colorClass="text-blue-600" onClick={() => onNavigateToActivity('InProgress')} />
                <WidgetCard title="Escalated" value={metrics.escalated} icon={<ExclamationCircleIcon />} subtext="Requires attention" colorClass="text-red-600" onClick={() => onNavigateToActivity('Escalated')} />
                <WidgetCard title="Total Completed" value={metrics.completed} icon={<CheckBadgeIcon />} subtext="Submitted in both passes" colorClass="text-slate-600" onClick={() => onNavigateToActivity('Completed')} />
            </div>
