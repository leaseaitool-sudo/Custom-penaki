...
const InteractiveMap = () => {
    ...
    const [selectedLoc, setSelectedLoc] = useState<typeof MOCK_LOCATIONS[0] | null>(null);
    ...
    if (!isLoaded) {
        return (
            <div className="w-full h-[400px] lg:h-[600px] rounded-2xl md:rounded-3xl overflow-hidden bg-slate-100 animate-pulse flex items-center justify-center border border-slate-200">
                <div className="flex flex-col items-center gap-3">
                    <div className="w-8 h-8 md:w-10 md:h-10 border-4 border-indigo-200 border-t-indigo-600 rounded-full animate-spin"></div>
                    <p className="text-sm md:text-base text-slate-400 font-bold uppercase tracking-widest">Initializing Map Engine...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="w-full h-[400px] lg:h-[600px] rounded-2xl md:rounded-3xl overflow-hidden shadow-2xl border border-slate-200 relative isolate group bg-slate-50">
            {/* Map UI Overlay */}
            <div className="absolute top-4 left-4 right-4 md:right-auto md:w-72 z-10 bg-white/95 backdrop-blur-md p-4 rounded-xl border border-white/50 shadow-lg animate-slide-up pointer-events-none">
                ...
                {MOCK_LOCATIONS.map(loc => (
                    <Marker
                        ...
                        icon={{
                            ...
                            fillColor: getPinColor(loc.status),
                            fillOpacity: 1,
                            strokeWeight: 2,
                            strokeColor: '#FFFFFF',
                            scale: selectedLoc?.id === loc.id ? 10 : 7,
                        }}
                    />
                ))}
                    ...
                    >
                        <div className="p-1 min-w-[200px]">
                            <div className="flex justify-between items-center mb-2">
                                <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${selectedLoc.status === 'Active' ? 'bg-green-100 text-green-700' : selectedLoc.status === 'Expired' ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'}`}>
                                    {selectedLoc.status === 'Expiring Soon' ? 'Expiring' : selectedLoc.status}
                                </span>
                                <span className="text-[10px] text-slate-400 font-medium">Ref #{selectedLoc.id}00</span>
                            </div>
                            <h4 className="font-bold text-slate-900 text-sm mb-0.5">{selectedLoc.title}</h4>
                            <p className="text-xs text-slate-500 mb-3">{selectedLoc.city}, {selectedLoc.type}</p>
                            <div className="pt-2 border-t border-slate-100 flex justify-between items-center">
                                <span className="text-xs font-bold text-indigo-600">{selectedLoc.rent}</span>
